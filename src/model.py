...
class LSTMTransFormer(Raw_model):
    def __init__(self, FLAGS,model_name,  optimizer,  num_layers,  num_heads, dff, rate, binary, unique_labels,  fill_cont):
        ...
        # self.enc_layers = [EncoderLayer(self.d_model, num_heads, dff, rate,  fill_cont, name='encoder_layer_'+str(n_l)) for n_l in range(len(num_layers))]
        self.enc_layers = [EncoderLayer(self.d_model, num_heads, dff, rate,  fill_cont, name='encoder_layer_'+str(n_l) if n_l > 0 else 'encoder_layer') for n_l in num_layers]
        # self.enc_layers = [EncoderLayer(self.d_model, num_heads, dff, rate,  fill_cont, name='encoder_layer')]
    ...
    def create_model(self, graph=False):
        ...
        x = layers.Dense(64, activation='relu', name='intermediate_dense')(x)
